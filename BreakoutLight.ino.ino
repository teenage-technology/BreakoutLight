/*
  ...
  Parts needed:
      Ardunio UNO
      Adapted for Elegoo 2.8 " TFT LCD display.
     Elegoo 2.8" TFT LCD Touch Display Arduino Shield or compatible

  This example code is in the public domain.

  Modified 07 11 2020
  By Enrique Albertos
  Modified 04/10/2023 by Owen Van Vooren (I modified it like 0.01%. Thanks for the awesome code, Enrique!


*/ 

//#define DEMO_MODE
//Uncomment ^DEMO_MODE to see a demo.
...
void updateLives(int lives, int remainingLives) {
  ...
  for (int i = 0; i < lives; i++) {
    tft.fillCircle((1 + i) * 15, 15, 5, RED);
  }

  for (int i = 0; i < remainingLives; i++) {
    tft.fillCircle((1 + i) * 15, 15, 5, GREEN);
  }
...
void drawPlayer(game_type * game, game_state_type * state) {
  // paint
  tft.fillRect(state->playerx, state->bottom, game->playerwidth, game->playerheight, BLACK);
...
void drawBall(int x, int y, int xold, int yold, int ballsize) {
  ...
  //if (xold != x && yold != y) {
  if (xold <= x && yold <= y) {
    tft.fillRect(xold , yold, ballsize, y - yold, WHITE);
    tft.fillRect(xold , yold, x - xold, ballsize, WHITE);
  } else if (xold >= x && yold >= y) {
    tft.fillRect(x + ballsize , yold, xold - x, ballsize, WHITE);
    tft.fillRect(xold , y + ballsize, ballsize, yold - y, WHITE);
  } else if (xold <= x && yold >= y) {
    tft.fillRect(xold , yold, x - xold, ballsize, WHITE);
    tft.fillRect(xold , y + ballsize, ballsize, yold - y, WHITE);
  } else if (xold >= x && yold <= y) {
...
void touchToStart() {
  drawBoxedString(-5, 200, "   BREAKOUT", 3, YELLOW, WHITE);
  drawBoxedString(-11, 240, "    TOUCH TO START", 2, RED, WHITE);
  while (waitForTouch() < 0) {}
}

void gameOverTouchToStart() {
  drawBoxedString(5, 180, "  GAME OVER", 3, YELLOW, WHITE);
  drawBoxedString(11, 220, "  TOUCH TO START", 2, RED, WHITE);
  while (waitForTouch() < 0) {}
}
...
void updateScore (int score) {
  ...
  drawBoxedString(tft.width() - 50, 6, buffer, 2, WHITE, BLACK);
}
...
void hitBrick(game_state_type * state, int xBrick, int yBrickRow) {
  ...
  drawBrick(state, xBrick, yBrickRow, BLACK);
  delay(16);
...
void clearDialog(gameSize_type gameSize) {
  ...
  tft.fillRect(gameSize.x, gameSize.y, gameSize.width, SCORE_SIZE, BLACK);
}
